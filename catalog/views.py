...
class SongListView(ListView):
    ...
    context_object_name = "songs"  # 模板中使用 songs 取得当前页歌曲
    paginate_by = 20

    def get_queryset(self):
        # 同时取得歌手，避免展示每首歌时再次查询。
        return Song.objects.select_related("artist")
...
def search(request: HttpRequest) -> HttpResponse:
    ...
    if form.is_valid():
        ...
        paginator = Paginator(results, 20) # 把搜索结果交给分页器，每页显示 20 条。
        page_obj = paginator.get_page(request.GET.get("page")) # 用户指定查看的搜索结果特定页码
